...
const { addRandomId, checkSendToken, createToken } = require('./forms/formEngine');
// const Universe = require('./../models/universe');

loginRouter.post('/access', async (request, response) => {

    const result = {};
    let check, browserId, randomId;
    if(request.body.from === 'admin') {
        ...
        randomId = request.body.randomId;
        const formToken = request.body.formToken;
        ...
        if(randomId) addRandomId(request.session, randomId);
        if(formToken) result.formToken = await createToken(request.session);
...
loginRouter.post('/', async (request, response) => {
    ...
    const sendTokenOk = await checkSendToken(request.session, body.token, body.sendToken);
    if(!sendTokenOk) {
